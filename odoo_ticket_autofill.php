<?php
/**
 * Lee un ticket de Odoo y devuelve únicamente datos reales disponibles.
 * No modifica Odoo ni la base de datos local.
 */
session_start();
ob_start();
...
function zgJson(array $data, int $status = 200): void {
...
function zgFail(string $message, int $status = 400): void {
    zgJson(['ok' => false, 'error' => $message], $status);
}
...
if (empty($_SESSION['panel_ok']) && empty($_SESSION['zgroup_tecnicos_ok'])) {
    zgFail('La sesión venció.', 403);
}
if ($_SERVER['REQUEST_METHOD'] !== 'GET') zgFail('Método no permitido.', 405);

$ticketRef = trim((string)($_GET['ticket_ref'] ?? ''));
if (!preg_match('/^\d{1,15}$/', $ticketRef)) zgFail('Ticket inválido.');
...
try {
    ...
    if (!is_array($rows) || empty($rows[0]) || !is_array($rows[0])) zgFail('Ticket no encontrado.', 404);
    $r = $rows[0];
    $ticketId = (int)($r['id'] ?? 0);
    ...
    if ($partnerId > 0) {
        try {
            ...
            if (is_array($partnerRows) && isset($partnerRows[0]) && is_array($partnerRows[0])) {
                ...
                $partnerAddress = zgClean((string)($p['contact_address_complete'] ?? ''));
                if ($partnerAddress === '') {
                    $state = zgReadableValue($p['state_id'] ?? '', ['type'=>'many2one']);
                    $country = zgReadableValue($p['country_id'] ?? '', ['type'=>'many2one']);
                    $partnerAddress = zgClean(implode(', ', array_filter([$p['street'] ?? '', $p['street2'] ?? '', $p['city'] ?? '', $state, $p['zip'] ?? '', $country], fn($v) => trim((string)$v) !== '')));
                }
    ...
    $defs = [
        'reporte' => ['numero de reporte','n de reporte','reporte','cotizacion','orden de servicio','orden de trabajo','service order','report number'],
    ...
    $reporte = preg_replace('/\D+/', '', $picked['reporte']['value']);
    if (strlen($reporte) < 6 || strlen($reporte) > 15) $reporte = zgDigitsReport($combined, $ticketRef);

    $direccion = $picked['direccion']['value'];
    if ($direccion === '') $direccion = zgLineValue($combined, ['Dirección del servicio','Dirección','Direccion','Ubicación','Ubicacion','Lugar del servicio']);
    ...
    if ($contenedor === $ticketRef || $contenedor === $reporte) $contenedor = '';
    ...
    zgJson(['ok' => true, 'data' => $data, 'datos_odoo' => $details]);
} catch (Throwable $e) {
    zgFail('No se pudo consultar Odoo.', 500);
}
